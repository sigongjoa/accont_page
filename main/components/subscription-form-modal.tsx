...
export function SubscriptionFormModal({ isOpen, onClose, onSubmit, subscription }: SubscriptionFormModalProps) {
  ...
  useEffect(() => {
    logger.debug('SubscriptionFormModal: useEffect - initialData changed', { subscription, isOpen });
    if (subscription) {
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    logger.debug('SubscriptionFormModal: handleSubmit called', { formData });
    ...
    onClose();
    logger.info(`SubscriptionFormModal: Subscription ${formData.serviceName} saved successfully.`);
  };
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button type="button" variant="outline" onClick={() => { onClose(); logger.debug('SubscriptionFormModal: Cancel button clicked'); }}>
              취소
            </Button>
            <Button type="submit" onClick={() => logger.debug('SubscriptionFormModal: Submit button clicked')}>{subscription ? "구독 업데이트" : "구독 추가"}</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
